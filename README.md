...
Опфатено: if (item.getBarcode() != null), else throw new RuntimeException("No barcode!"), if (item.getDiscount() > 0), sum += item.getPrice() * item.getDiscount(), if (item.getPrice() > 300 && item.getDiscount() > 0 && item.getBarcode().charAt(0) == '0'), sum -= 30 - (8,11,12,13,14,15,16)
Тест случај 5: Предмет без попуст
allItems = [new Item("Item1", "123456", 100, 0)], payment = 100
true
Опфатено: if (item.getDiscount() > 0), sum += item.getPrice(), if (sum <= payment) - (13,18
